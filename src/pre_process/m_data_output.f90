    ...
    subroutine s_write_serial_data_files(q_cons_vf) ! -----------
        ...
        if (n == 0) then
            do i = 1, sys_size
                ...
                do j = 0, m
                    call s_convert_to_mixture_variables(q_cons_vf, j, 0, rho, gamma, pi_inf)
                    ...
                    if (((i >= cont_idx%beg) .and. (i <= cont_idx%end)) &
                        ...
                        call s_compute_pressure(q_cons_vf(E_idx)%sf(j, 0),  &
                            0.5d0*(q_cons_vf(mom_idx%beg)%sf(j, 0)**2.d0)/rho, pi_inf, gamma, pres)
